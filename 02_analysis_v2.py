...
def main():
    ...
    ap.add_argument("--outdir", default="/content/drive/MyDrive/Wasko_JPIM/output_v2/results")
    ...
    log("\nComparison with the published Table 3 (Model 2):")
    log("Differences are EXPECTED on the recoded data; this is not an error.")
    got = {
    ...
    for k, v in got.items():
        ...
        ok = abs(v - exp) < 0.005
        ok_all &= ok
        log(f"    {'same' if ok else 'diff'} {k:12s} now {v:+.3f}  published {exp:+.3f}")
    log("  --> identical to the published table." if ok_all else
        "  --> differs from the published table, as expected after recoding.")
    ...
    log("Full console output is saved to analysis_log.txt.")
